...
for REPEAT in "${REPEATS[@]}"; do
    ...
    python $PYTHON_SCRIPT $CONFIG \
        ...
        --model.retrieval_augmentation.key_tag image,resnet_50,classification \
        --model.retrieval_augmentation.value image \
        --model.retrieval_augmentation.value_tag image,resnet_50,classification \
    ...
    for TEMPERATURE in "${TEMPERATURES[@]}"; do
        for ALPHA in "${ALPHAS[@]}"; do
            echo "Running repeat $REPEAT with batch size $PYTHON_SCRIPT --temperature $TEMPERATURE --alpha $ALPHA"
            python $PYTHON_SCRIPT $CONFIG \
            --model.input-modality $INPUT_MODALITY \
