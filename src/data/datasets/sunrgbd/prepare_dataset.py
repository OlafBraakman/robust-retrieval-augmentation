# -*- coding: utf-8 -*-
# From: https://github.com/Barchid/RGBD-Seg/blob/master/dataloaders/sunrgbd/prepare_dataset.py
import os
...
if __name__ == '__main__':
    ...
    # # Load everything for the segmentation dataset
    ...
    classification_depth_dir_train = []
    classification_scene_dir_train = []

    classification_img_dir_test = []
    classification_depth_dir_test = []
    classification_scene_dir_test = []


    # Ignore classes that are not part of the scene classification task
    for _, meta in tqdm(enumerate(SUNRGBDMeta)):
        meta_dir = '/'.join(meta.rgbpath.split('/')[:-2])
        real_dir = meta_dir.split('/n/fs/sun3d/data/SUNRGBD/')[1]
        scene_path = os.path.join(real_dir, 'scene.txt')

        # Skip all datapoints that are not part of the 
        with open(os.path.join(output_path, 'SUNRGBD', scene_path), 'r') as f:
            scene = f.readline()
        if not scene in SUNRBDBase.SCENES:
            continue
        ...
        if meta_dir in split_train:
            classification_img_dir_train.append(os.path.join('SUNRGBD', rgb_path))
            classification_depth_dir_train.append(os.path.join('SUNRGBD', depth_bfx_path))
            classification_scene_dir_train.append(os.path.join('SUNRGBD', scene_path))

        else:
            classification_img_dir_test.append(os.path.join('SUNRGBD', rgb_path))
            classification_depth_dir_test.append(os.path.join('SUNRGBD', depth_bfx_path))
            classification_scene_dir_test.append(os.path.join('SUNRGBD', scene_path))
    ...
    _write_list_to_file(classification_depth_dir_train, 'classification_train_depth.txt')
    ...
    _write_list_to_file(classification_depth_dir_test, 'classification_test_depth.txt')
