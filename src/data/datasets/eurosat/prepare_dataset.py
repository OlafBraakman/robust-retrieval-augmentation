...
if __name__ == '__main__':
    ...
    parser = ap.ArgumentParser(
        description='Prepare EuroSAT dataset for classification.')
    ...
    image_dir = os.path.join(output_path, '')
    # Get all images
    all_images = glob(os.path.join(image_dir, '*/*.jpg'))  # Assuming images are in class-named subfolders
    ...
    _write_list_to_file(train_labels, 'classification_train_scene.txt')

    _write_list_to_file(test_images, 'classification_test_rgb.txt')
    _write_list_to_file(test_labels, 'classification_test_scene.txt')
