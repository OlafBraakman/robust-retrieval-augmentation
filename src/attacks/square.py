...
class Square(Attack):
    ...
    def __init__(self, num_classes, eps=0.1, alpha=2/255, max_iters=30, device="cuda", **kwargs) -> None:
        ...
        self.original_accuracy aa= Accuracy(task='multiclass', num_classes=num_classes, average='micro').to(device)
    ...
    def run(self, model, dataloader):
        ...
        for (X,y), _ in tqdm(dataloader):
            ...
            self.square_attack_l2(model, X, y)
    ...
    def square_attack_l2(self, model, x, y, corr_classified, eps, n_iters, p_init, metrics_path, targeted, loss_type):
    ...
    def square_attack_linf(self, model, x, y, corr_classified, eps, n_iters, p_init, metrics_path, targeted, loss_type):
